...
- 不要把原视频改成二创版本、换壳版本或新故事方向。用户要的是反推结果，不是二创改写。
...
以下 13 项是你的分析框架和上下文维度。不要在顶层单独输出 13 个模块，但必须把这些信息消化、合并、整理进 reverseResult 模块里。reverseResult 的含义是“反推结果”：忠实记录、整理和反推原视频本身，不是二创版本、改编版本或换壳版本。

用户拿到 reverseResult 后，应该能尽量还原理解这条原视频：故事结构、人物状态、场景调度、光影色彩、镜头语言、美术氛围、情绪节奏、AI 生成注意事项都要完整可用。不要写空泛形容词，不要只写摘要。
...
12. 反推还原注意事项：提取原视频必须保留的画面元素、人物状态、动作、光影、镜头、情绪节奏和不应改动部分。
...
- “反推还原注意事项”里的“必须保留”指画面中真实出现的构图、主体、动作、服装轮廓、光影关系、镜头语言、情绪节奏等，不要擅自改动。
- sourceOrderedScript 必须忠实整理原视频的剧情和画面顺序，不要写成二创版本。
...
最终只输出两个顶层模块。Return valid JSON only. Use this exact schema:
{
  ...
  "finalSummary": {
    ...
    "faithfulRetentionPoints": "",
    "aiGenerationFailureRisks": ""
  }
}

写作要求：
- reverseResult 必须是主输出，它在页面上显示为“反推结果”。信息量要充分，不要只写几句摘要。
- reverseResult.title 只给原视频内容起一个忠实描述标题，不要加入用途类、改编类或版本类后缀。
- reverseResult.projectBasics 要整合：视频类型、题材类型、整体风格、叙事节奏、情绪基调、视觉关键词、原视频可见信息。用条目式写清楚。
- reverseResult.storySynopsis 写 300-500 字，必须忠实概括原视频可见剧情，包括：开场状态、冲突出现、情绪变化、关键转折、结尾钩子或情绪落点。不要改成新故事。
- reverseResult.dramaticStructure 要按场景/功能段落拆解剧情，包括场景名称、地点、时间、人物、事件、人物目标、冲突点、情绪变化、画面重点、剧情作用。至少 3 个段落。
- reverseResult.sceneBreakdown 要把重要场景的空间、美术、前中后景、主表演区、可拍路径、材质、道具、生活痕迹和象征意义按原视频事实整理清楚。写到别人能理解原视频场景。
- reverseResult.characterProfiles 要为重要人物写小传：临时称呼、年龄感、身份、外貌、服装造型、气质、当前情绪、隐含过往、目标、弱点、关系、表演关键词。不能只有“主角/配角”。
- reverseResult.visualStyleGuide 要总结影像质感、色彩倾向、构图、镜头距离、画面密度、景深、真实感/电影感/广告感/短剧感/AI 感比例。要能指导画面生成。
- reverseResult.lightingColorGuide 要总结主光源方向、光线硬软、阴影、高光、暗部、色温、主色、辅助色、情绪色彩和剧情功能。写清楚光从哪来、怎么打、为什么这么打。
- reverseResult.productionDesignGuide 要反推原视频的美术设计原则，包括场景搭建、材质、道具、空间层次和画面符号。不要换成新的美术方案。
- reverseResult.shotLanguageGuide 要按视频时间顺序拆解关键镜头，包含镜头编号、时间段、景别、机位、角度、运动、焦距感、构图主体、人物动作、情绪、声音/环境音、叙事作用。至少 6 个镜头段落；如果关键帧少，就根据相邻帧合理推断。
- reverseResult.emotionalRhythmCurve 要写清起始情绪、第一次变化、升高点、压抑点、爆发点、结尾余味、观众吸引点、爽点/虐点/悬疑点/反转点/记忆点。要像剪辑节奏说明。
- reverseResult.dialogueVoiceoverSubtitleNotes 要提取对白、旁白、字幕、屏幕 UI、标题文字、关键信息文字；看不清或无音频就标注“疑似”或“未提供音频，无法确认”。
- reverseResult.reverseEngineeringNotes 要提炼：必须保留的画面元素、人物状态、动作、光影、镜头语言、情绪节奏、不建议改动部分、无法确认部分。不要写“可改动部分”，除非画面本身确实存在不确定性。
- reverseResult.sourceOrderedScript 必须使用标准短剧剧本格式：
  场景编号｜地点｜时间｜人物
  画面描述：
  人物动作：
  人物情绪：
  ...
  声音：
  转场：
- sourceOrderedScript 要按原视频可见顺序忠实整理，不要写成二创版本，不要自行添加原视频没有的场景和台词。
- finalSummary 必须用导演视角回答：真正吸引人的核心、最应该学习什么、哪些原视频元素必须忠实保留、AI 重新生成最容易偏离原片的地方。
...
function normalizeAnalysis(parsed, frameNames, rawText) {
  ...
  const reverseResult =
    parsed.reverseResult ||
    parsed.reusableStoryVersion ||
    directorPackage.reverseResult ||
    directorPackage.reusableStoryVersion ||
    directorPackage.reusableScriptVersion ||
    parsed.reusableScriptVersion ||
    {};
  if (!reverseResult.reverseEngineeringNotes && reverseResult.aiVideoRecreationNotes) {
    reverseResult.reverseEngineeringNotes = reverseResult.aiVideoRecreationNotes;
  }
  if (!reverseResult.sourceOrderedScript && reverseResult.standardShortDramaScript) {
    reverseResult.sourceOrderedScript = reverseResult.standardShortDramaScript;
  }

  const finalSummary = directorPackage.finalSummary || parsed.finalSummary || {};
  if (!finalSummary.faithfulRetentionPoints && finalSummary.newStoryAdaptationDirection) {
    finalSummary.faithfulRetentionPoints = finalSummary.newStoryAdaptationDirection;
  }
  ...
  return {
    ...
    directorPackage,
    reverseResult,
    reusableStoryVersion: reverseResult,
    reusableScriptVersion: reverseResult,
    finalSummary,
    prompts: {},
    rawText
  };
